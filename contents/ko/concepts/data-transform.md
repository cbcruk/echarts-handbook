...
`데이터 변환`은 Apache ECharts<sup>TM</sup> 5부터 지원되었습니다. ECharts에서 `데이터 변환`이라는 용어는 사용자가 제공한 소스 데이터와 변환 함수로부터 새로운 데이터를 생성하는 것을 의미합니다. 이 기능은 사용자가 선언적 방식으로 데이터를 처리할 수 있게 하고, 이런 종류의 작업을 "바로 사용 가능한" 상태로 만들기 위해 몇 가지 일반적인 "변환 함수"를 제공합니다. (문맥의 일관성을 위해 "transformation"보다는 "transform"이라는 명사 형태를 사용합니다).

데이터 변환의 추상적 공식은 다음과 같습니다: `outData = f(inputData)`, 여기서 변환 함수 `f`는 `filter`, `sort`, `regression`, `boxplot`, `cluster`, `aggregate`(예정) 등이 될 수 있습니다.
이러한 변환 메서드의 도움으로 사용자는 다음과 같은 기능을 구현할 수 있습니다:

- 데이터를 여러 시리즈로 분할
- 통계를 만들고 결과를 시각화
- 시각화 알고리즘을 데이터에 적용하고 결과 표시
- 데이터 정렬
- 빈 데이터나 특수한 데이터를 제거하거나 선택
- ...

## 데이터 변환 시작하기

ECharts에서 데이터 변환은 [dataset](~${optionPath}#dataset) 개념을 기반으로 구현됩니다. 데이터세트 인스턴스에서 [dataset.transform](${optionPath}#dataset.transform)을 구성하여 이 데이터세트가 이 `transform`에서 생성되어야 함을 나타낼 수 있습니다. 예를 들어:
