...
이 문서는 Apache ECharts<sup>TM</sup> 스타일 커스터마이징에 대한 다양한 접근 방식의 개요를 제공합니다. 예를 들어, 그래픽 요소와 라벨의 색상, 크기, 그림자를 구성하는 방법 등입니다.

> "스타일"이라는 용어는 데이터 시각화의 관례를 따르지 않을 수도 있지만, 널리 사용되고 이해하기 쉽기 때문에 이 문서에서 사용합니다.

아래 접근 방식들이 소개됩니다. 이들의 기능은 겹칠 수도 있지만, 서로 다른 시나리오에 적합합니다.

- 테마(Theme)
- 색상 팔레트(Color Palette)
- 명시적 스타일 커스터마이징 (itemStyle, lineStyle, areaStyle, label, ...)
- 시각적 인코딩 (visualMap 컴포넌트)
