...
`dataset`은 데이터 관리 전용 컴포넌트입니다. 각 시리즈마다 `series.data`에 데이터를 설정할 수 있지만, ECharts 4부터는 `dataset`을 사용하여 데이터를 관리하는 것을 권장합니다. 이렇게 하면 데이터를 여러 컴포넌트에서 재사용할 수 있고 "데이터와 설정"의 분리를 편리하게 할 수 있습니다. 결국 데이터는 런타임에 가장 자주 변경되는 부분이고, 다른 설정들은 대부분 변경되지 않기 때문입니다.

## **series**에서 **데이터** 정의

`series`에서 데이터를 정의하는 경우, 예를 들어:
